...
ccpar = 1.d-1; %Limit at which parameters have converged
...
bstssr=oldssr;
bstans(1:numparms) = answers(1:numparms);



for cur1loop = 1 : maxloops + 1, % main convergence loop
 
    % store answers in temporary array
    tmpans(1:numparms) = answers(1:numparms);
    
    % now initialize the matrix equation to all zeros
    JTJ = zeros(numparms,numparms);
    JTR = zeros(numparms,1);
    partials = zeros(3,1);
    ...
    [e,s,t,idx] = symsv(JTJ,JTR,numparms);
    ...
    % if old and new ssr differ by less than 1 part in 1000
    % we might be converged. Also need to check parameters differ by
    % less than 1 part in 1000. If both conditions true we are done
    % and want to break out of convergence loop
    
    converged = 1;
    
    if (zz < ccvar) % If variance has converged
        % check individual parameters
        for i=1 : numparms
            if tmpans(i) ~= 0
                zz2 = abs(answers(i)/tmpans(i)-1);
                if(zz2 > ccpar)
                    converged = 0;
                end
            end
